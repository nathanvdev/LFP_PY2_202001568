...
def UserMessage():
    ...
    if CorrectSyntax and CorrectLexicon:
        if Comando[0].Lexeme == 'RESULTADO':
            ...
            GameList, nombre = Partidos(Comando, nombre)
...
def Partidos(Team: List, nombre: str):
    GameList = []
    ...
    for x in range(len(Team)):
        if Team[x].Lexeme == '-ji':
            ...
            nombre += '\n Desde Jornada {}'.format(FromJourney)
        elif Team[x].Lexeme == '-jf':
            ToJourney = int(Team[x+1].Lexeme)
            nombre += '\n Hasta Jornada {}'.format(ToJourney)
