import CSV, Analizador, Objects, webbrowser
...
def chatGUI():
    ...
    menubutton.menu.add_command(label = "Limpiar Log de Errores", background="#00a884", foreground="#ffffff", command=lambda:[Analizador.LexicalError.clear(), Analizador.SyntacticalError.clear()]) 
...
def UserMessage():
    global TokenList, LexicalError, SyntacticalError
    ...
    if CorrectSyntax and CorrectLexicon:
        ...
        if Comando[0].Lexeme == 'RESULTADO':
            ...
            JornadaList = []
            for Partido in PartidosList:
                if Partido.Temp == Comando[3].Lexeme and Partido.Journey == int(Comando[1].Lexeme):
        ...
        elif Comando[0].Lexeme == 'TOP':
            mensaje = 'El Top {} de la temporada {} fue \n'.format(Comando[1].Lexeme.lower(), Comando[3].Lexeme)
            TablaList = Tabla(Comando[3].Lexeme)
            TopList = []
            Top = 5
            TopAdd = 0
            if len(Comando) > 4 :
                Top = int(Comando[5].Lexeme)
            
            if Comando[1].Lexeme == 'SUPERIOR':
                for x in range(Top):
                    TopList.append(TablaList[x])

            elif Comando[1].Lexeme == 'INFERIOR':
                for i in range(len(TablaList) -1, -1, -1):
                    if TopAdd < Top:
                        TopList.insert(0, TablaList[i])
                        TopAdd += 1

            Equipo : Objects.Tabla
            for Equipo in TopList:
                mensaje += '{}. {} \t pts: {}\n'.format(Equipo.POS, Equipo.Name, Equipo.PTS)
    ...
    if CorrectLexicon == False:
        SendMessage('BOT', '!!! Error Lexico !!! "{}" comando no encontrado'.format(LexicalError[-1].Lexeme))
    if CorrectSyntax == False:
        SendMessage('BOT', '!!! Error Sintactico !!! "{}" orden incorrecto'.format(SyntacticalError[-1].Lexeme))
