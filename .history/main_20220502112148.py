...
def Tabla(temp):
    ...
    for partido in PartidosList:
        ...
        if partido.Temp == temp:
            ...
            LocalTeam.Blogo = partido.Icon1
            VisitTeam.Blogo = partido.Icon2            LocalTeam.PJ += 1
            VisitTeam.PJ += 1
    ...
    for Equipo in TablaList:
        ...
        toFind = str(Equipo.Name)
        toFind = toFind.lower().replace(' ','-')
        s1 = toFind.replace("ñ", "#").replace("Ñ", "%")
        toFind = unicodedata.normalize("NFKD", s1)\
        ...
        for line in logos:
            ...
            if toFind in LogoCode[3][0]:
                Equipo.logo = LogoCode[3][0]
                Equipo.logo = 'shield-sprite xl {}'.format(Equipo.logo)
                Equipo.Blogo = True
                break
...
def Partidos(Team):
    ...
    for Match in PartidosList:
        if Team == Match.Local or Team == Match.Visit:
